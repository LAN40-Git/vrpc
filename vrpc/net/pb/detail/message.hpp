...
public:
    ...
    static constexpr uint32_t MIN_MESSAGE_SIZE = sizeof(seq_) + sizeof(status_code_)+ sizeof(err_msg_size) + sizeof(payload_size_) + sizeof(check_sum_);
public:
    [[nodiscard]]
    static auto parse_from(const void* data, uint32_t size) -> std::optional<RpcResponseMessage> {
        ...
        auto& err_msg_size = message.err_msg_size;
    ...
    [[nodiscard]]
    auto bytes_size() const -> uint32_t {
        return MIN_MESSAGE_SIZE + err_msg_size + payload_size_;
    }
